...
FireScanner::FireScanner()
    : FireScanner(pins::servo::HORIZONTAL, pins::sensors::FLAME) {}

FireScanner::FireScanner(int servoPin, int flameSensorPin)
    : _servoPin(servoPin), _flameSensorPin(flameSensorPin) {}

void FireScanner::begin() {
    pinMode(_flameSensorPin, INPUT);

    ledcSetup(_pwmChannel, kServoFrequencyHz, kServoPwmResolutionBits);
    ledcAttachPin(_servoPin, _pwmChannel);

    _currentAngle = clampAngle(_centerAngle);
...
void FireScanner::setCenterAngle(uint8_t centerAngle) {
    if (centerAngle <= 180) {
...
void FireScanner::setSensorActiveLow(bool activeLow) {
...
void FireScanner::updateSweep(unsigned long currentMillis) {
    if (currentMillis - _lastStepMillis < _stepIntervalMillis) {
        return;
    }
...
void FireScanner::updateFireReading() {
    int rawValue = digitalRead(_flameSensorPin);
    bool detected = _sensorActiveLow ? rawValue == LOW : rawValue == HIGH;
    FireDirection previousDirection = _fireDirection;
    ...
    if (detected != _fireDetected || _fireDirection != previousDirection) {
        _hasNewData = true;
    }
...
void FireScanner::writeServoAngle(uint8_t angle) {
    uint16_t pulseMicros = pulseForAngle(angle);
    uint32_t duty = (static_cast<uint32_t>(pulseMicros) * kMaxDuty) / kServoPeriodMicros;
    ledcWrite(_pwmChannel, duty);
}

uint16_t FireScanner::pulseForAngle(uint8_t angle) const {
    uint32_t pulseRange = _maxPulseMicros - _minPulseMicros;
    return _minPulseMicros + ((pulseRange * angle) / 180);
}

FireDirection FireScanner::directionForAngle(uint8_t angle) const {
    const uint8_t centerWindow = 15;

    if (angle + centerWindow < _centerAngle) {
        return FireDirection::Left;
    }

    if (angle > _centerAngle + centerWindow) {
